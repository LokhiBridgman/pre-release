...
# Q EXAMPLE: block when trie it ws to put into @
import random
import os
#imports functionsf    
...
def Main():
    ...
    Score = 0
    while Again == "y":
        #lets player play infinitely
        Filename = input("Press Enter to start a standard puzzle or enter name of file to load: ")
        #either load an existing puzzle or specific one
        if len(Filename) > 0:
            #if a file actually is entered
            MyPuzzle = Puzzle(Filename + ".txt")
        else:
            MyPuzzle = Puzzle(8, int(8 * 8 * 0.6))
            #creates an 8x8 grid
        Score = MyPuzzle.AttemptPuzzle()
        #player plays puzzle using attempt method puzzle of class
        #displays puzzle grid, user's score is updated based off of moves
    ...
    def __init__(self, *args):
        if len(args) == 1:
            ...
            for Count in range(1, self.__GridSize * self.__GridSize + 1):
                if random.randrange(1, 101) < 90: #why is it 101
                    C = Cell()
                else:
                    C = BlockedCell()
    ...
    def __LoadPuzzle(self, Filename):
        try:
            with open(Filename) as f:
                ...
                self.__SymbolsLeftd = int(f.readline().rstrip())
        except:
